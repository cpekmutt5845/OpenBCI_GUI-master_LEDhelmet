...
//ome code
CaiShenHighLevelAPI api; //add api
//end here
...
void initSystem() {
  ...
  //choose what ch you want to stream
  selectElectrodePerformed();
  
  //please don't forget to write config
  writeElectrodeConfigPerformed();
  
  //set helmet to start mode
  startPerformed();
...
//ome code
    void writeElectrodeConfigPerformed() {                                                        
        try {
            println("Write Electrode Config\n");
    ...
    void selectElectrodePerformed() {        
        api.enableElectrode("Fp1");
        api.enableElectrode("Fp2");
        api.enableElectrode("C3");
        api.enableElectrode("C4");
        api.enableElectrode("F7");
        api.enableElectrode("F8");
        api.enableElectrode("O1");
        api.enableElectrode("O2");
        //can update to 16ch by add code to this function 
    }    
    
    
   void startPerformed() {                                         
        try{
            println("Set Mode Start\n");
            if (api.start())
            println("OK, starting\n");
            else
            println("ERROR\n");
   ...
   void connectPerformed() { 
        ...
        try{
            api.connect("10.10.10.1"); //default is "10.10.10.1"
            println("Connected\n");
        } catch (Exception ex) {
            println(ex.getMessage()+"\n");
        }
